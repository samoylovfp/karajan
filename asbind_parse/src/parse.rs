...
fn field_def(input: &mut &str) -> Result<Field> {
    seq!(
        Field{
            name: alphanumeric1.map(ToString::to_string).context(Label("field name")),
            _ : (space0, ':', space0),
            r#type: field_type,
...
fn struct_def(input: &mut &str) -> Result<Struct> {
    seq!(
        Struct {
            ...
            name: alphanumeric1.map(ToString::to_string).context(Label("struct name")),
...
pub fn parse(s: &str) -> anyhow::Result<KrjFile> {
    Ok(KrjFile {
        structs: repeat(0.., struct_def).context(Label("Repeated struct definition"))
            .parse(s)
            .map_err(|e| anyhow!("{e:?}"))?,
    })
}
